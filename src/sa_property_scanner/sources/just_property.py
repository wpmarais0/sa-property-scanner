"""Just Property source adapter.

Just Property uses the PropertyWeb platform (same as Harcourts).
...
class JustPropertySource(SourceAdapter):
    """Adapter for just.property (PropertyWeb platform)."""
    ...
    def parse(self, html: str) -> list[RawListing]:  # type: ignore[override]
        ...
        for card in cards:
            try:
                ...
                url_abs = self._make_absolute_url("https://www.just.property", href)
                ...
                # Extract property type from title, e.g. "3 Bedroom Freehold For Sale in Vredenburg"
                property_type = None
                if title:
                ...
                self.logger.warning("Failed to parse Just Property card: %s", exc)

        self.logger.info("Just Property parsed %d listings", len(listings))
        return listings
